...
func (s *SmartContract) CreateUser(ctx contractapi.TransactionContextInterface,
	user string, pubkey string, id string, initialBalance int) error {
	...
	pubkeyExists, err := s.UserExists(ctx, pubkey)
	if err != nil {
		return fmt.Errorf("failed to get user info")
	}
	if pubkeyExists {
	...
	userIndex, err := ctx.GetStub().CreateCompositeKey(index, []string{user, pubkey, id})
	if err != nil {
...
func (s *SmartContract) CreateContract(ctx contractapi.TransactionContextInterface, contractJSON string) error {
	...
	if !exists {
		return fmt.Errorf("provider with id %s does not exist", sla.Details.Provider.ID)
	}
	...
	if !exists {
		return fmt.Errorf("client with id %s does not exist", sla.Details.Client.ID)
	}
