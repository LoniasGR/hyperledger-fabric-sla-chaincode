...
function channel_command_group() {
  ...
  if [ "${COMMAND}" == "init" ]; then
    log "Initializing admins"
    channel_init
    log "🏁 - Channel is initialized."

  elif [ "${COMMAND}" == "create" ]; then
    if [ $# -lt 2 ]; then
      log "Usage: create {CHANNEL_NAME} {ORG_NR}"
      exit 1
    fi
    log "Creating channel \"$1\":"
...
function create_genesis_block() {
  ...
  envsubst <"${PWD}"/config/org0/configtx-template.yaml >"${TEMP_DIR}"/configtx.yaml

  FABRIC_CFG_PATH=${TEMP_DIR} \
    configtxgen \
    -profile "Org${ORG_NR}ApplicationGenesis" \
