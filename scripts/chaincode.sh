...
function deploy_chaincode() {
  ...
  if [ "${CHAINCODE_BUILDER}" == "ccaas" ]; then
    ...
    launch_chaincode      "${org_nr}" "${cc_name}" "${CHAINCODE_ID}" "${CHAINCODE_IMAGE}"
  fi

  activate_chaincode      "$org_nr" "${cc_name}" "${cc_package}"
}
...
function activate_chaincode() {
  ...
  install_chaincode   "${org_nr}" "${cc_package}"
  approve_chaincode   "${org_nr}" "${cc_name}" "${CHAINCODE_ID}"
  commit_chaincode    "${org_nr}" "${cc_name}"
}
...
function launch_chaincode_service() {
  ...
  cat "kube/${org}/${org}-cc-template.yaml" \
    ...
    | exec kubectl -n "$org_ns" apply -f -

  kubectl -n "$org_ns" rollout status "deploy/${org}${peer}-ccaas-${cc_name}"
